...
from ropnroll.core import loader, scanner, security
...
def xfg_pe(tmp_path):
    ...
    oh.dll_characteristics = oh.dll_characteristics | int(
        lief.PE.OptionalHeader.DLL_CHARACTERISTICS.GUARD_CF
    )
    lc = b.load_configuration
    lc.guard_flags = int(lc.guard_flags) | int(
        lief.PE.LoadConfiguration.IMAGE_GUARD.XFG_ENABLED
    )
...
def test_xfg_detected_and_distinct_from_cfg(xfg_pe):
    ...
    report = security.build_report(
        img, scanner.scan_image(img, scanner.ScanOptions(max_insns=4))
    )
...
def cet_pe(tmp_path):
    ...
    dbg_rva, dbg_sz = struct.unpack_from(
        "<II", d, datadir + 6 * 8
    )  # dir index 6 = debug
    ...
    struct.pack_into(
        "<I", d, e + 12, 20
    )  # Type = IMAGE_DEBUG_TYPE_EX_DLLCHARACTERISTICS
