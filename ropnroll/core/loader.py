...
class Image:
    ...
    entrypoint: int
    image_base: int         # base LIEF assumed while parsing (the PE's own preferred ImageBase)
    base_known: bool = True # is `image_base` (and every address derived from it) a real,
    ...
    def rebase(self, new_base: int) -> "Image":
        ...
        return Image(
            ...
            entrypoint=self.entrypoint + delta, image_base=new_base, base_known=True,
            os=self.os, segments=segs,
...
def _lief_arch_to_ropnroll(binary: lief.Binary) -> tuple[str, int, bool]:
    ...
    if binary.format != lief.Binary.FORMATS.PE:
        raise ValueError(f"unsupported binary format: {binary.format} (ropnroll only reads PE)")
...
def load(path: str) -> Image:
    ...
    return Image(
        ...
        little_endian=le, pie=pie, entrypoint=entry, image_base=image_base,
        base_known=not pie, os="windows",
