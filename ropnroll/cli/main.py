...
def _clobbers_str(eff, sp_reg: str = "") -> str:
    ...
    if eff.mem_writes:
        parts.append(f"+{len(eff.mem_writes)} mem write" + ("s" if len(eff.mem_writes) != 1 else ""))
...
def _sort_note(by: str) -> str:
    return "by address" if by == "address" else "best-first (fewest instructions, then address)"
...
def _gadget_line(addr: str, text: str, n_insns: int, eff, sp_reg: str, text_width: int = 42) -> str:
    line = (f"{addr}  {escape(text.ljust(text_width))}  "
        ...
        with console.status("[dim]analyzing effects...[/dim]"):
            with open(args.out, "w") as f:
                for g in gadgets:
                    ...
                    f.write(f"{_addr_str(g.address, pool)} : {g.text:<42} "
                             f"instrs={g.n_insns} clobbers={_clobbers_str(eff, sp_reg)} sp={_sp_str(eff)}\n")
        console.print(f"[green]wrote {len(gadgets)} gadgets to {args.out}[/green], sorted {_sort_note(args.sort)}")
        return

    shown = gadgets[: args.limit]
    header = f"gadgets: {args.binary}" + (f" matching /{args.regex}/" if args.regex else "")
    console.print(f"[bold]{header}[/bold]")
    for g in shown:
        eff = pool.effect_of(g)
        console.print(_gadget_line(_addr_str(g.address, pool), g.text, g.n_insns, eff, sp_reg))
    console.print(f"[bold]{len(gadgets)}[/bold] gadgets total, sorted {_sort_note(args.sort)}"
...
def cmd_search(args):
    ...
    for g, eff, field in results:
        console.print(_gadget_line(_addr_str(g.address, pool), g.text, g.n_insns, eff, sp_reg))
    console.print(f"[bold]{len(results)}[/bold] match(es), best-first (fewest instructions, then address)")
...
def cmd_pivot(args):
    ...
    for p in pivots[: args.limit]:
        t.add_row(_addr_str(p.gadget.address, pool), escape(p.gadget.text), str(p.gadget.n_insns), p.kind)
    console.print(t)
    console.print(f"[bold]{len(pivots)}[/bold] pivot(s) found"
...
def cmd_jop(args):
    ...
    t = Table(title="JOP dispatcher gadgets (self-advancing jmp/call-through-register, best-first)")
    t.add_column("address")
    t.add_column("gadget")
    t.add_column("instrs", justify="right")
    t.add_column("reg")
    ...
    for d in disp[: args.limit]:
        t.add_row(_addr_str(d.gadget.address, pool), escape(d.gadget.text), str(d.gadget.n_insns),
                   d.reg, f"0x{d.advance:x}")
    console.print(t)
...
def _note_unresolved(chain):
    ...
    if unresolved:
        ...
        console.print(f"[yellow]chain has symbolic addresses for: {mods} (base not known yet -- "
...
def _sub(sub, name, *, help, description=None, epilog=None, parents=()):
    return sub.add_parser(
        ...
        parents=list(parents), formatter_class=_Fmt,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--max-insns", type=int, default=6, metavar="N",
                         help="maximum instructions per gadget (default: 6)")
    common.add_argument("--no-rop", action="store_true",
                         help="disable ROP (return-terminated) gadgets")
    ...
    common.add_argument("--no-sys", action="store_true",
                         help="disable syscall-terminated gadgets")
    common.add_argument("--bad-bytes", metavar="HEX",
                         help="byte values a gadget's address may not contain, as hex, "
                              "e.g. 000a0d")
    common.add_argument("--no-cache", action="store_true",
                         help="disable the persistent on-disk gadget-scan and semantic-effect caches")
    common.add_argument("--jobs", "-j", type=int, default=None, metavar="N",
    ...
    s = _sub(sub, "security", parents=[common],
    ...
    s = _sub(sub, "scan", parents=[common],
    ...
    s.add_argument("--regex", metavar="PATTERN", help="only show gadgets whose disassembly matches this regex")
    s.add_argument("--sort", choices=["quality", "address"], default="quality",
                    help="'quality' ranks fewest-instructions-first, a decent proxy for the "
                         ...
                         "(default: quality)")
    s.add_argument("--limit", type=int, default=50, metavar="N", help="max gadgets to print (default: 50)")
    s.add_argument("--out", metavar="PATH", help="write every matching gadget to this file instead "
                                                  "of printing (ignores --limit)")
    s.set_defaults(func=cmd_scan)

    s = _sub(sub, "search", parents=[common],
    ...
    s.add_argument("--query", required=True, metavar="EXPR",
                    help='effect to match, e.g. "rdi=rax+8", "[rbx]=rax", "rax=0"')
    s.add_argument("--limit", type=int, default=20, metavar="N", help="max matches to print (default: 20)")
    s.set_defaults(func=cmd_search)

    s = _sub(sub, "pivot", parents=[common],
              help="find stack-pivot gadgets",
              epilog="example:\n  ropnroll pivot ./target")
    s.add_argument("binary", nargs="+", help="binaries to pool gadgets from")
    s.add_argument("--limit", type=int, default=30, metavar="N", help="max pivots to print (default: 30)")
    s.set_defaults(func=cmd_pivot)

    s = _sub(sub, "jop", parents=[common],
              ...
              epilog="example:\n  ropnroll jop ./target")
    s.add_argument("binary", nargs="+", help="binaries to pool gadgets from")
    s.add_argument("--limit", type=int, default=30, metavar="N", help="max dispatchers to print (default: 30)")
    s.set_defaults(func=cmd_jop)

    s = _sub(sub, "call", parents=[common],
    ...
    s.add_argument("binary", nargs="+", help="binaries to pool gadgets from; the target symbol is "
                                              "resolved against these binaries' exports")
    s.add_argument("--target", required=True, metavar="ADDR|SYMBOL",
                    help="address (0x...) or symbol name to call")
    s.add_argument("--args", default="", metavar="N,N,...",
                    help="comma-separated integer arguments, e.g. 0x1000,0,0")
    s.add_argument("--bytes-before-chain", type=int, default=None, metavar="N",
    ...
    s.add_argument("--emit", choices=["pwntools", "raw", "c", "json"], help="export the chain in this format")
    s.add_argument("--out", metavar="PATH", help="write the emitted chain here instead of stdout")
    s.add_argument("--verify", action="store_true", help="concretely verify the chain with Unicorn emulation")
